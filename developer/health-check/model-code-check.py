...
class ModelCodeChecker:
    ...
    def __init__(self, repo_root: Optional[str] = None, verbose: bool = False, strict: bool = False):
        ...
        # File and directory paths (support both charts/litellm and charts/platform/charts/litellm)
        if os.path.exists(os.path.join(self.repo_root, "charts/litellm")):
            self.litellm_chart_dir = os.path.join(self.repo_root, "charts/litellm")
        else:
    ...
    def _detect_repo_root(self, candidate: Optional[str]) -> str:
        ...
        while current and current != os.path.dirname(current):
            if os.path.exists(os.path.join(current, "charts/litellm")) or os.path.exists(
                os.path.join(current, "charts/platform/charts/litellm")
            ):
                return current
        ...
        cwd = os.getcwd()
        if os.path.exists(os.path.join(cwd, "charts/litellm")) or os.path.exists(
            os.path.join(cwd, "charts/platform/charts/litellm")
        ):
            return cwd
    ...
    def _check_required_files(self) -> bool:
        required_paths = [
            ("Models Directory", self.models_dir, True, True),
            ("Secrets Template", self.secrets_yaml_path, False, True),
            ("Deployment Template", self.deployment_yaml_path, False, True),
            ("ConfigMap Template", self.configmap_yaml_path, False, True),
            ("Lago Callback", self.lago_callback_path, False, False),  # Optional
        ]

        all_ok = True
        for name, path, is_dir, is_mandatory in required_paths:
    ...
    def _validate_single_model_file(self, file_path: str, target_env: Optional[str] = None):
        ...
        if environments is None:
            # In this Helm chart, environments is optional and defaults to all valid environments
            environments = sorted(list(self.VALID_ENVIRONMENTS))
        elif not isinstance(environments, list) or len(environments) == 0:
            self.add_issue(
                "ERROR",
                "Schema",
                "'environments' must be a non-empty list of environment names if specified",
                file_path,
            )
            environments = []
        else:
